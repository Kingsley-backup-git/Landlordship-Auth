...
<div className='w-full py-2'>
    <div className={`flex gap-x-2 rounded-xl ${open ? "" : "hover:bg-[#1C1C1C0D]"} items-center p-2 cursor-pointer`} onClick={()=> Dropdown()}>
    <HiOutlineBuildingOffice2 className='text-[#1C1C1C] min-w-[20px] min-h-[20px] max-w-[20px] w-[100%] h-[100%]'/>
    <h1 className='flex-1 text-[#1C1C1C] text-xs font-[400]'>Properties</h1>
   {open && <MdOutlineKeyboardArrowDown className='text-[#1C1C1C33] min-w-[20px] min-h-[20px] max-w-[20px] w-[100%] h-[100%]'/>}
    ...
    {open && <div className=' ps-4 flex flex-col gap-y-1'>
<Link onClick = {()=> setOpen(true)} href = "/dashboard/unit" className={`text-[#1C1C1C] hover:bg-[#1C1C1C0D] rounded-xl cursor-pointer text-xs font-[400] p-2  ${pathname.includes("/unit") ? "bg-[#1C1C1C0D]": null}`}>Units</Link>
<Link onClick = {()=> setOpen(true)}  href = "/dashboard/overview" className='text-[#1C1C1C] hover:bg-[#1C1C1C0D] rounded-xl text-xs font-[400] p-2'>Buildings</Link>
...
<h1 className='text-[#1C1C1C] text-xs font-[400] flex-1'>Maintenance</h1>
