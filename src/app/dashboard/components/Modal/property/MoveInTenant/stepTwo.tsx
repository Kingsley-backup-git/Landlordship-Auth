...
<p className="text-sm text-center mt-2 font-[400] text-[#00000066]">Select the tenant from the dropdown menu.</p>
...
<h1 className='text-[#00000066] text-xs pt-1 font-normal'>Tenants</h1>
<div className='w-full flex relative py-1 cursor-pointer ' onClick={openAddNew}>
    <>
   {tenants.length < 1 ? <div className='flex-1 text-black font-[400] text-sm'>Choose a tenant</div> :
<div className="flex gap-x-2 overflow-auto flex-1 items-center pe-8">
   {tenants.map((tenant, index)=> {
return  <div key={index} className='bg-[#FAF4DF] rounded-lg flex items-center gap-x-1 px-1 py-[2px]'>
<h1 className='text-xs text-black font-[400] whitespace-nowrap'>{tenant.firstname} {tenant.lastname}</h1> 
...
<Button onClick={()=> stepHandler(1)} classname='bg-[#0000000A] flex items-center justify-center gap-x-2 max-w-[170px] w-full rounded-xl py-2 px-4'>
<FaAngleLeft className='text-[#00000066] text-base'/>
  <h1 className="text-sm font-[400] text-black">Previous</h1>
</Button>

<Button onClick={()=> stepHandler(6)} classname='bg-[#1D3639] flex items-center gap-x-2 justify-center max-w-[170px] w-full rounded-xl py-2 px-4'>
