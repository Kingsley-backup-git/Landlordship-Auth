...
export default function StepSix({header, stepHandler, sendInviteMutation}:{header:string, stepHandler : (num:number)=> void;sendInviteMutation : any}) {
  useEffect(()=> {
...
<h1 className="font-semibold text-xl text-black text-center">Shared lease</h1>
...
<div className='flex-[25%] text-[#00000066]  font-normal text-xs'>Properties</div>
<div className=' text-[#00000066] font-normal flex-[25%] text-xs text-center'>Tenant Email</div>
...
<div className=' text-[#8A8CD9] flex-[25%] justify-center font-normal text-xs flex items-center gap-x-1'><div className='w-[5px] h-[5px] bg-[#8A8CD9] rounded-full'></div><h1>{tenant?.status === "pending" && "In Progress"}</h1></div>
</div>
}) }
...
<Button onClick={()=> stepHandler(4)} classname='bg-[#0000000A] flex flex-1 items-center justify-center gap-x-2 rounded-xl py-2 px-4'>
<FaAngleLeft className='text-[#00000066] text-base'/>
  <h1 className="text-sm font-[400] text-black">Add new Email</h1>
</Button>

<Button onClick={()=> stepHandler(9)} classname='bg-[#1D3639] flex flex-1 items-center gap-x-2 justify-center rounded-xl py-2 px-4'>
