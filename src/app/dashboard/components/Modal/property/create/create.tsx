    ...
    formData.append("yearBuilt", values.yearBuilt.toString());
        formData.append("uniqueId", values.uniqueId)
         formData.append("stateAddress", values.stateAddress)
          ...
          if (values.propertyType === "individual" || values.propertyType === "multi-unit") {
...
<Button type='button' onClick={()=>stepHandler(-1)} classname=' bg-transparent border-[.5px] border-[#0000001A] text-sm text-black font-[400] rounded-lg py-2 px-4 mt-4' text="Cancel"/>

<Button disabled = {disabled} type='submit' onClick={(e: React.MouseEvent<HTMLButtonElement>)=> createPropertyHandler(e)} classname={` ${disabled || Object.keys(formik.errors).length > 0  ? "bg-gray-400 cursor-not-allowed" : "bg-black cursor-pointer"}text-white  text-sm font-[400]  rounded-lg py-1 px-2 mt-4`} text="Create"/>
</div>
