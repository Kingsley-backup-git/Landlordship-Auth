...
export default function Timeline() {
  ...
  return (
    <div className="sm:bg-[#F9F9FA] bg-white rounded-2xl pt-6 lg:col-span-6 order-[1] md:col-span-7 col-span-12 sm:px-6 px-[15px] flex flex-col sm:h-[272px] xs:h-[286px] h-[350px]">
      ...
      <div
        className={`grid grid-flow-col  pt-4 justify-between overflow-x-auto  ${styles.overflow}  gap-x-3 `}
      >
        {dates.map((date, index) => {
          return (
            <div
              key={index}
              className={`py-1 col-span-4 cursor-pointer px-2 max-w-fit w-[100%] rounded-lg ${date.active && "bg-black"}`}
            >
              <h1
                className={`text-[#00000066] text-xs font-[400] ${date.active && "text-white"}`}
              >
                {date.day}
              </h1>
              <p
                className={`font-semibold text-sm text-black ${date.active && "text-white"}`}
              >
                {date.date}
              </p>
            </div>
      ...
      <div
        className={`overflow-y-auto flex-1  flex flex-col ${styles.overflow}`}
      >
          ...
          return (
            <div key={index} className="pt-2 flex gap-x-2 ps-2">
              ...
              <div className="flex-1">
                ...
                <p className="text-[#00000066] text-xs font-[400]">
                  {singletimeline.time}
                </p>
              </div>
            </div>
