...
<div className='flex-[23%] text-black font-[400] text-xs flex items-center gap-x-2'>
   {property.tenantId?.email && <Image src = {Img1} className = "w-[24px] h-[24px] rounded-full" alt = "tenant-img" width={24} height = {24} />}
...
<div className='flex-[2%]'>
 <BsThreeDots  className={`text-lg  ${(show === property._id) ? "visible":"invisible"} text-black`}   onClick={(event) => {
...
<div className='sm:flex sm:max-w-[300px] w-full col-span-12 sm:ms-auto hidden items-center gap-x-2 mt-3'>
  ...
  <div     onClick={() => setPage((old) => Math.max(old - 1, 1))} className='py-1  text-center cursor-pointer rounded-lg border-[#0000001A] border-[0.5px]  sm:flex-[.5] flex-1'>
