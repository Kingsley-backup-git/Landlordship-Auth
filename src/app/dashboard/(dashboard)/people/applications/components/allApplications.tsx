  ...
  return (
    <div className="grid lg:grid-cols-3 1md:grid-cols-2 md:grid-cols-2 2sm:grid-cols-2 grid-cols-1 sm:grid-cols-1 mt-5 gap-x-5 gap-y-6 sm:gap-y-8">
      {allApplications.map((application, index) => {
        return (
          <div className="sm:bg-[#F9F9FA] bg-white rounded-2xl p-5" key={index}>
            <div className="flex items-center justify-between gap-x-1">
              <div>
                <h1 className="font-semibold text-sm text-black">
                  {application.propertyName}
                </h1>
                <p className="text-xs font-[400] text-[#00000066] mt-[.5px]">
                  {application.address}
                </p>
              </div>

              <div className="bg-[#0000000A] min-w-[35px] max-w-[35px] h-[35px] rounded-lg"></div>
            </div>
            ...
            <div className="flex justify-between my-4 gap-x-1 items-center">
              ...
              <div
                className={`font-[400] text-xs flex items-center gap-x-1 ${application.status === "In Progress" ? "text-[#8A8CD9]" : application.status === "Complete" ? "text-[#94E9B8]" : application.status === "Pending" ? "text-[#92BFFF]" : application.status === "Approved" ? "text-[#FFDB56]" : application.status === "Rejected" ? "text-[#00000066]" : null}`}
              >
                <div
                  className={`w-[7px] h-[7px] rounded-full ${application.status === "In Progress" ? "bg-[#8A8CD9]" : application.status === "Complete" ? "bg-[#94E9B8]" : application.status === "Pending" ? "bg-[#92BFFF]" : application.status === "Approved" ? "bg-[#FFDB56]" : application.status === "Rejected" ? "bg-[#00000066]" : null}`}
                ></div>
            ...
            <div className="bg-[#0000000A] w-[100%] rounded-full h-[2px]">
              <div
                ...
                className={`h-full ${application.status === "In Progress" ? "bg-[#8A8CD9]" : application.status === "Complete" ? "bg-[#94E9B8]" : application.status === "Pending" ? "bg-[#92BFFF]" : application.status === "Approved" ? "bg-[#FFDB56]" : application.status === "Rejected" ? "bg-[#00000066]" : null}`}
              ></div>
            </div>
            ...
            <div className="flex justify-between items-center mt-2">
              ...
              <h1 className="text-black text-xs font-[400]">
                {application.completionRate}%
              </h1>
            </div>
          </div>
        );
