...
export default function TenantAcc({accountType, AccountTypeHandler} : {accountType : string, AccountTypeHandler : (val :string)=> void;}) {
  return (
   
       <div className={`${accountType === "tenant" ? "border-[2px]" : "" } cursor-pointer relative border-[#1D3639] rounded-2xl p-6 bg-[#F9F9FA] flex gap-x-2`} onClick={()=> AccountTypeHandler("tenant")}>
