...
{dates.map((date, index)=> {
    return <div key={index} className={`py-1 col-span-4 cursor-pointer px-2 max-w-fit w-[100%] rounded-lg ${date.active && "bg-[#e3572d]"}`}>
        <h1 className={` text-[10px] font-[400] ${date.active ? "text-white" : "text-[#4d5253]"}`}>{date.day}</h1>
        <p className={`font-semibold text-[11px] text-white  ${date.active && "text-white"}`}>{date.date}</p>
        </div>
})}
</div>
...
<div className='flex-1'>
    <h1 className='text-white font-[400] text-[9px]'>{singletimeline.req}</h1>
    <p className='text-[#4d5253] text-[8px] font-[400]'>{singletimeline.time}</p>
</div>
</div>
})}
</div>
